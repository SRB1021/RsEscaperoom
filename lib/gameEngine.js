...
export class EscapeRoomGame {
  constructor(container, theme, callbacks, network = null) {
  ...
  _buildPaintingAndSafe() {
    ...
    this.safeInteractable = {
      mesh: safe,
      getPrompt: () => {
        if (this.state.safeOpen) return null;
        return this.state.hasFuse ? `[E] Open ${theme.safe.label}` : `[E] Try ${theme.safe.label}`;
      },
      onInteract: () => {
        if (this.state.safeOpen) return;
        if (!this.state.hasFuse) {
          this.audio.denied();
          this.cb.onToast(theme.fuse.noPowerText);
          return;
        }
        this.audio.click();
        this.keypadDigits = "";
  ...
  _buildFuse() {
    ...
    fuse.position.set(0, 0.3, -3.5);
    this.stageGroup.add(fuse);
    this.fuseMesh = fuse;
    ...
    this.stageGroup.add(hitbox);

    this.fuseInteractable = {
      mesh: hitbox,
      getPrompt: () => `[E] Pick up ${theme.fuse.label}`,
      onInteract: () => {
        this._takeFuse();
      },
    };
  ...
  _buildDoor() {
    ...
    this.doorInteractable = {
      ...
      onInteract: () => {
        ...
        if (isFinalStage) this._winGame(elapsed);
  ...
  // safe, or unlocks the door, everyone in the room sees it. Each method
  ...
  _takeTool({ broadcast = true, silent = false } = {}) {
    if (this.state.hasTool) return;
    this.state.hasTool = true;
    if (this.toolMesh) this.toolMesh.visible = false;
    this.audio.pickup();
    const label = this.theme.tool.label;
    ...
    if (broadcast) this._sendAction("hasTool");
  }

  _takeFuse({ broadcast = true, silent = false } = {}) {
    if (this.state.hasFuse) return;
    this.state.hasFuse = true;
    if (this.fuseMesh) this.fuseMesh.visible = false;
    this.audio.pickup();
    const label = this.theme.fuse.label;
    if (!this.inventoryItems.includes(label)) {
      this.inventoryItems.push(label);
      this.cb.onInventory([...this.inventoryItems]);
    }
    if (!silent) this.cb.onToast(`The ${label} has been picked up.`);
    this.interactables = this.interactables.filter((i) => i !== this.fuseInteractable);
    if (broadcast) this._sendAction("hasFuse");
  }

  _openCrate({ broadcast = true, silent = false, instant = false } = {}) {
    if (this.state.crateOpened) return;
    this.state.crateOpened = true;
    this.audio.creak();
    const tag = this.tagMesh;
    if (tag) {
      tag.visible = true;
    ...
    if (broadcast) this._sendAction("crateOpened");
  }

  _movePainting({ broadcast = true, silent = false, instant = false } = {}) {
    if (this.state.paintingMoved) return;
    this.state.paintingMoved = true;
    this.audio.creak();
    const cover = this.coverMesh;
    const safe = this.safeMesh;
    const finish = () => {
      ...
      if (this.safeInteractable && !this.interactables.includes(this.safeInteractable)) {
    ...
    if (broadcast) this._sendAction("paintingMoved");
  }

  _openSafe({ broadcast = true, silent = false } = {}) {
    if (this.state.safeOpen) return;
    this.state.safeOpen = true;
    this.audio.unlock();
    if (this.keyMesh) this.keyMesh.visible = true;
    if (this.keyInteractable && !this.interactables.includes(this.keyInteractable)) {
      this.interactables.push(this.keyInteractable);
    }
    if (!silent) this.cb.onToast(`The ${this.theme.safe.label} clicks open.`);
    if (broadcast) this._sendAction("safeOpen");
  }

  _takeKey({ broadcast = true, silent = false } = {}) {
    ...
    if (!silent) this.cb.onToast(`The ${label} has been picked up.`);
    if (broadcast) this._sendAction("hasKey");
  }

  _advanceStage({ broadcast = true, silent = false, animateDoor = true } = {}) {
    if (broadcast) this._sendAction("stageAdvance");
    const run = () => {
      if (!silent) this.cb.onToast(this.theme.door.advanceToast);
      this.stageIndex += 1;
      this._buildStage();
    };
    if (animateDoor && this.doorHinge) {
      this.audio.unlock();
      this.audio.creak();
      const dur = 1.3;
      this._tween(dur, (p) => {
        this.doorHinge.rotation.y = -p * 1.9;
      });
      // Wait for the door-swing tween to fully finish before tearing down
      // and rebuilding the stage — otherwise the animation gets cut off
      // mid-swing, which reads as a glitch.
  ...
  _winGame(elapsed, { broadcast = true, animateDoor = true } = {}) {
    if (broadcast) this._sendAction("win");
    const run = () => {
      this.state.won = true;
      this.audio.victory();
  ...
  _sendAction(kind, payload = null) {
    if (this.network) this.network.send({ t: "action", kind, payload });
  }
  ...
  _onNetworkMessage(msg) {
    ...
    else if (msg.t === "action") this._applyNetworkAction(msg.kind);
  }

  _applyNetworkAction(kind) {
    const opts = { broadcast: false };
    if (kind === "hasTool") this._takeTool(opts);
    else if (kind === "crateOpened") this._openCrate(opts);
    else if (kind === "hasFuse") this._takeFuse(opts);
    else if (kind === "paintingMoved") this._movePainting(opts);
    else if (kind === "safeOpen") this._openSafe(opts);
    else if (kind === "hasKey") this._takeKey(opts);
    else if (kind === "stageAdvance") this._advanceStage(opts);
    else if (kind === "win") {
      const elapsed = (performance.now() - this.startTime) / 1000;
      this._winGame(elapsed, opts);
    }
  }

  // Fast-forwards a newly-joined client to the room's current progress:
  // jump straight to the right stage, then silently/instantly replay
  // whichever steps are already done, then place the other players.
  applyJoinSnapshot(msg) {
    if (msg.stageIndex > 0) {
      this.stageIndex = msg.stageIndex;
      this._buildStage();
    }
    const opts = { broadcast: false, silent: true, instant: true };
    if (msg.state.hasTool) this._takeTool(opts);
    if (msg.state.crateOpened) this._openCrate(opts);
    if (msg.state.hasFuse) this._takeFuse(opts);
    if (msg.state.paintingMoved) this._movePainting(opts);
    if (msg.state.safeOpen) this._openSafe(opts);
    if (msg.state.hasKey) this._takeKey(opts);
  ...
  getHint() {
    ...
    if (!s.hasFuse) return "Something in this room needs power before it'll do anything. Keep looking.";
