...
export class EscapeRoomGame {
  ...
  _addCrateStack(x, z, base = "#33261a", band = "#6b5a3a", ry = 0) {
    const mat = new THREE.MeshStandardMaterial({ map: TX.crateTexture(base, band), roughness: 0.9 });
  ...
  _addBarrel(x, z, color = "#5a4530", count = 1) {
    const mat = new THREE.MeshStandardMaterial({ map: TX.crateTexture(color, "#2a2016"), roughness: 0.85 });
  ...
  _addWallTool(x, y, z, ry, tool = "pickaxe", color = "#8a8f94") {
    const woodMat = new THREE.MeshStandardMaterial({ map: TX.woodTexture("#4a3220") });
    const metalMat = new THREE.MeshStandardMaterial({ map: TX.metalTexture(color), metalness: 0.6, roughness: 0.4 });
  ...
  _addChainMount(x, y, z, ry) {
    const woodMat = new THREE.MeshStandardMaterial({ map: TX.woodTexture("#3a2c1a") });
    const metalMat = new THREE.MeshStandardMaterial({ map: TX.metalTexture("#6b6f74"), metalness: 0.7, roughness: 0.4 });
    const group = new THREE.Group();
  ...
  _addPedestal(x, z, h = 0.4, base = "#1a1a1a", band = "#333") {
    const mat = new THREE.MeshStandardMaterial({ map: TX.crateTexture(base, band), roughness: 0.8 });
  ...
  _addLanternProp(x, y, z, glassColor = "#dff2ff") {
    const metalMat = new THREE.MeshStandardMaterial({ map: TX.metalTexture("#2a2a2a"), metalness: 0.6, roughness: 0.4 });
