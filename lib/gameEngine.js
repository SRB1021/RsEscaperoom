...
export class EscapeRoomGame {
  ...
  _buildDesk() {
    ...
    const plaqueMat = new THREE.MeshStandardMaterial({
      map: TX.plaqueTexture(theme.code.slice(0, 2), theme.plaque),
    });
  ...
  _buildRackAndCrate() {
    ...
    const tagMat = new THREE.MeshStandardMaterial({ map: TX.tagTexture(theme.code.slice(2), theme.tag) });
  ...
  keypadSubmit() {
    if (this.keypadDigits === this.theme.code) {
