 ...
 * @property {object} plan - the randomized layout (see lib/roomPlan.js), generated by the host
 * @property {object[]} roomStates - one state object per room in the plan
 * @property {Map<string, {ws: import('ws').WebSocket, color: string, x: number, z: number, yaw: number}>} players
 */
    ...
    ws.on("message", (raw) => {
      ...
      if (msg.t === "host") {
        ...
        const room = {
          theme: msg.theme,
          plan,
          roomStates: (plan?.rooms || []).map(freshRoomState),
          players: new Map(),
        };
      ...
      if (msg.t === "action") {
        const kind = msg.kind;
        const roomIndex = msg.roomIndex;
        if (kind === "win") {
          // no per-room state to flip; just relay
        } else {
          const state = room.roomStates[roomIndex];
          if (state && Object.prototype.hasOwnProperty.call(state, kind)) {
            state[kind] = true;
          }
        }
        broadcast(room, { t: "action", roomIndex, kind, payload: msg.payload || null }, playerId);
      }
    });
